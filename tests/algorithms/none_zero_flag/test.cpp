...
std::vector<int>
test_none_zero_flag_random_input(std::mt19937& rng) {
  std::uniform_int_distribution<int> size_dist(0, 52);
  std::uniform_int_distribution<int> value_dist(-8, 8);
...
std::filesystem::path
test_none_zero_flag_styio() {
  return styio::testing::algorithms::styio_program(
    "none_zero_flag", "none_zero_flag.styio");
}

} // namespace

TEST(StyioCppReferenceEquivalence, test_none_zero_flag) {
  std::mt19937 rng(0x0E207E);

  for (int iteration = 0; iteration < 180; ++iteration) {
    const std::vector<int> input = test_none_zero_flag_random_input(rng);
    const std::string stdin_text =
      styio::testing::algorithms::format_i32_list(input) + "\n";
    const std::string expected = test_none_zero_flag_cpp_output(input);

    const styio::testing::algorithms::CommandResult actual =
      styio::testing::algorithms::run_styio_program(
        test_none_zero_flag_styio(), stdin_text);
