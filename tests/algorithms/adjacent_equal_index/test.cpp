...
std::vector<int>
test_adjacent_equal_index_random_input(std::mt19937& rng) {
  std::uniform_int_distribution<int> size_dist(0, 48);
  std::uniform_int_distribution<int> value_dist(-12, 12);
...
std::filesystem::path
test_adjacent_equal_index_styio() {
  return styio::testing::algorithms::styio_program(
    "adjacent_equal_index", "adjacent_equal_index.styio");
}

} // namespace

TEST(StyioCppReferenceEquivalence, test_adjacent_equal_index) {
  std::mt19937 rng(0xADACE17);

  for (int iteration = 0; iteration < 180; ++iteration) {
    const std::vector<int> input = test_adjacent_equal_index_random_input(rng);
    const std::string stdin_text =
      styio::testing::algorithms::format_i32_list(input) + "\n";
    const std::string expected = test_adjacent_equal_index_cpp_output(input);

    const styio::testing::algorithms::CommandResult actual =
      styio::testing::algorithms::run_styio_program(
        test_adjacent_equal_index_styio(), stdin_text);
