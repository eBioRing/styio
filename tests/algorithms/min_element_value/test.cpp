...
std::vector<int>
test_min_element_value_random_input(std::mt19937& rng) {
...
std::filesystem::path
test_min_element_value_styio() {
  return styio::testing::algorithms::styio_program(
    "min_element_value", "min_element_value.styio");
}

} // namespace

TEST(StyioCppReferenceEquivalence, test_min_element_value) {
  std::mt19937 rng(0xA11E17);

  for (int iteration = 0; iteration < 180; ++iteration) {
    const std::vector<int> input = test_min_element_value_random_input(rng);
    const std::string stdin_text =
      styio::testing::algorithms::format_i32_list(input) + "\n";
    const std::string expected = test_min_element_value_cpp_output(input);

    const styio::testing::algorithms::CommandResult actual =
      styio::testing::algorithms::run_styio_program(
        test_min_element_value_styio(), stdin_text);
