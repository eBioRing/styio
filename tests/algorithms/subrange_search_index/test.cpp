...
std::filesystem::path
test_subrange_search_index_styio() {
  return styio::testing::algorithms::styio_program(
    "subrange_search_index", "subrange_search_index.styio");
}

std::string
format_subrange_search_index_input(const SubrangeSearchInput& input) {
...
TEST(StyioCppReferenceEquivalence, test_subrange_search_index) {
  std::mt19937 rng(0x5EA2C4);

  for (int iteration = 0; iteration < 160; ++iteration) {
    const SubrangeSearchInput input =
      test_subrange_search_index_random_input(rng);
    const std::string expected =
      test_subrange_search_index_cpp_output(input.haystack, input.needle);

    const styio::testing::algorithms::CommandResult actual =
      styio::testing::algorithms::run_styio_program(
        test_subrange_search_index_styio(),
        format_subrange_search_index_input(input));

    ASSERT_EQ(actual.exit_code, 0) << actual.stderr_text;
    EXPECT_EQ(actual.stdout_text, expected)
      << "input=" << format_subrange_search_index_input(input)
