...
std::vector<int>
test_minmax_pair_random_input(std::mt19937& rng) {
...
std::filesystem::path
test_minmax_pair_styio() {
  return styio::testing::algorithms::styio_program(
    "minmax_pair", "minmax_pair.styio");
}

} // namespace

TEST(StyioCppReferenceEquivalence, test_minmax_pair) {
  std::mt19937 rng(0xA1E1A3);

  for (int iteration = 0; iteration < 180; ++iteration) {
    const std::vector<int> input = test_minmax_pair_random_input(rng);
    const std::string stdin_text =
      styio::testing::algorithms::format_i32_list(input) + "\n";
    const std::string expected = test_minmax_pair_cpp_output(input);

    const styio::testing::algorithms::CommandResult actual =
      styio::testing::algorithms::run_styio_program(
        test_minmax_pair_styio(), stdin_text);
