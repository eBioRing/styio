...
std::vector<int>
test_exclusive_scan_sum_random_input(std::mt19937& rng) {
...
std::filesystem::path
test_exclusive_scan_sum_styio() {
  return styio::testing::algorithms::styio_program(
    "exclusive_scan_sum", "exclusive_scan_sum.styio");
}

} // namespace

TEST(StyioCppReferenceEquivalence, test_exclusive_scan_sum) {
  std::mt19937 rng(0xECCA11);

  for (int iteration = 0; iteration < 180; ++iteration) {
    const std::vector<int> input = test_exclusive_scan_sum_random_input(rng);
    const std::string stdin_text =
      styio::testing::algorithms::format_i32_list(input) + "\n";
    const std::string expected = test_exclusive_scan_sum_cpp_output(input);

    const styio::testing::algorithms::CommandResult actual =
      styio::testing::algorithms::run_styio_program(
        test_exclusive_scan_sum_styio(), stdin_text);
