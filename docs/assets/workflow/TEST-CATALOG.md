...
**任务语义失败用例（无 golden）：** `tests/milestones/m12/e01`–`e02`，CTest 名 `m12_err_e01_…` 与 `m12_err_e02_…`（标签 **`m12_semantic`**）。覆盖未声明 flow target 与同一 task 重复 pull。`ctest --test-dir build/default -L m12_semantic`。
