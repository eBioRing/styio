...
| `styio_resource_topology_test` | `tests/resource_topology_test.cpp`：覆盖编译器内部 RTG（resource topology graph）安全规则，包括写入目标 capability 拒绝、close-capable 资源所有权、流式背压边、state-owned hidden ledger、以及 `StyioHandleTable` `release_all` 关闭和 slot 回收。 | `ctest --test-dir build/default -L resource_topology` 或 `ctest --test-dir build/default -R '^StyioResourceTopology\\.'` |
...
| `styio_resource_topology_test` | `tests/resource_topology_test.cpp`：覆盖资源拓扑的安全子集，确保 close-capable source 不会无主进入 lowering、只读 source 不会被当作 sink、hidden ledger 只能由 state 拥有、背压关系进入图模型、handle table 批量释放不会泄漏或遗留 active slot。 | `ctest --test-dir build/default -L resource_topology` 或 `ctest --test-dir build/default -L safety` |
