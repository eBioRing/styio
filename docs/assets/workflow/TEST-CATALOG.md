...
**2026-05-09 修订：** M6 state-family spellings have been retired from active syntax.
...
**迁移边界：** 当前表格就是 M6 的 active acceptance set。旧 state-family
拼写不再按历史编号补正例；只保留 `tests/milestones/m6/e01`-`e04` 负例迁移诊断。
新增资源拓扑正例时应使用 Topology v2 写法并同步本表、`tests/CMakeLists.txt`
和相关 resource-topology unit tests。
...
| `readme_glimpse_job_parallel_signal` | `README.md` 的 `A Glimpse of Styio` 示例必须指向可运行的仓库内源文件；`example/job_parallel_signal.styio` 使用 `price_a, price_b <- @stdin : (f64, f64)` 拉取两行价格，使用 `||> [ ... ]` 启动并行 job、用 `?|` await 两个结果，并将组合信号写入 `@stdout`，stdout 必须匹配 `example/expected/job_parallel_signal.out`。 | `ctest --test-dir build/default -R '^readme_glimpse_job_parallel_signal$'` |
