...
24. Temporary ASTs produced during parser recovery and fallback must stay under local ownership until `ListAST`, `DictAST`, `BlockAST`, `MainBlockAST`, `RangeAST`, or equivalent adopters take them. Session-arena release does not run destructors for abandoned parser nodes, so exception paths that drop partially parsed literals or block statements can leak nested string storage even when the outer AST object memory comes from the arena.
