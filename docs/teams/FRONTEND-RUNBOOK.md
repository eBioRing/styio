...
24. Temporary ASTs produced during nightly literal fallback must stay under local ownership until `ListAST`, `DictAST`, `RangeAST`, or equivalent adopters take them. Session-arena release does not run destructors for abandoned parser nodes, so exception paths that drop partially parsed literals can leak nested string storage even when the outer AST object memory comes from the arena.
